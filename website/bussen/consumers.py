...
from bussen.services import get_player_from_cookie, decode_message
...
class GamePhase3Consumer(GroupConsumer):
    ...
    def execute_message(self, message: dict, player: Player):
        """Execute a message send by a player."""
        if "type" not in message.keys():
            return False
        elif message["type"] == "phase3_guess":
            if message["index"] == player.current_game.game.bus.current_card_index:
                correct = player.current_game.phase3_guess(message["guess"], player)
                if correct is not None:
                    if not correct:
                        async_to_sync(self.channel_layer.group_send)(
                            self.get_group_name(player.current_game),
                            {
                ...
                if player.current_game.phase == BusGameModel.PHASE_FINISHED:
                    async_to_sync(self.channel_layer.group_send)(
                        self.get_group_name(player.current_game),
                        {
                            "type": "send_group_message",
                            "text": json.dumps(
                                {
                                    "type": "celebrate",
                                    "url": reverse("bussen:game_room", kwargs={"game": player.current_game}),
                                }
                            ),
                        },
                    )
    ...
    @staticmethod
    def get_group_name(game: BusGameModel):
        """Get the group name."""
...
class GamePhase2Consumer(GroupConsumer):
    ...
    def execute_message(self, message, player):
        ...
        if "type" not in message.keys():
            ...
            if removed:
                async_to_sync(self.channel_layer.group_send)(
                    self.get_group_name(card_of_player.current_game),
                    {"type": "send_group_message", "text": json.dumps({"type": "refresh"})},
                )
                async_to_sync(self.channel_layer.group_send)(
                    self.get_group_name(player.current_game),
            ...
            if player.current_game.phase2_next_turn(message["index"]):
                if player.current_game.phase == BusGameModel.PHASE_2:
                    async_to_sync(self.channel_layer.group_send)(
                        self.get_group_name(player.current_game),
                        {"type": "send_group_message", "text": json.dumps({"type": "refresh"})},
                    )
...
class GamePhase1Consumer(GroupConsumer):
    ...
    def handle_phase1_answer(self, player: Player, value: int):
    ...
    def execute_message(self, message, player: Player):
...
class HubConsumer(GroupConsumer):
    ...
    def redirect_group(self, game):
        ...
        async_to_sync(self.channel_layer.group_send)(
            hub_websocket_id,
            {
                "type": "send_group_message",
                "text": json.dumps(
                    {"type": "game_redirect", "url": reverse("bussen:game_room", kwargs={"game": game})}
                ),
            },
        )
    ...
    def send_amount_of_players(self, game):
        ...
        response_dict = {
            ...
            "can_start": game.has_enough_players and game.phase == BusGameModel.PHASE_OPEN,
        }
        async_to_sync(self.channel_layer.group_send)(
